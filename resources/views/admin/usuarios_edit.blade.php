    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 flex items-center space-x-4">
            <div class="h-12 w-12 flex-shrink-0">
                <img class="h-12 w-12 rounded-full object-cover border-2 border-gray-100 shadow-sm"
        ...
        <form method="POST" action="{{ route('admin.usuarios.update', $usuario->id) }}" class="px-6 py-6 space-y-5">
            @csrf
            @method('PUT')

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Nombre de usuario <span class="text-red-500">*</span></label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Nombre real</label>
                    <input type="text" name="nombre_real"
                           value="{{ old('nombre_real', $usuario->nombre_real) }}"
                           class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Apellidos</label>
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Rol <span class="text-red-500">*</span></label>
                <div class="flex p-1 bg-gray-100 rounded-lg border border-gray-200 w-full">
                    @foreach(['usuario' => 'Usuario', 'artista' => 'Artista', 'admin' => 'Admin'] as $val => $label)
                    <label class="flex-1 text-center cursor-pointer">
                        <input type="radio" name="rol" value="{{ $val }}" class="sr-only peer"
                               {{ old('rol', $usuario->rol) === $val ? 'checked' : '' }}>
