...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 flex items-center space-x-4">
            ...
            <div>
                <h3 class="text-lg font-bold text-gray-800">Editar Álbum</h3>
                <p class="text-sm text-gray-500">{{ $album->artista->nombre_artistico ?? 'Sin artista' }}</p>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.albums.update', $album->album_id) }}" class="px-6 py-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text"
                           value="{{ $album->artista->nombre_artistico ?? 'Sin artista' }}"
                           disabled
                           class="w-full border border-gray-100 bg-gray-50 rounded-lg px-4 py-2.5 text-sm text-gray-500 cursor-not-allowed">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Fecha de lanzamiento</label>
                    <input type="text"
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Estado <span class="text-red-500">*</span></label>
                <div class="flex p-1 bg-gray-100 rounded-lg border border-gray-200 w-full">
                    @foreach(['publicado' => 'Publicado', 'borrador' => 'Borrador'] as $val => $label)
                    <label class="flex-1 text-center cursor-pointer">
                        <input type="radio" name="estado" value="{{ $val }}" class="sr-only peer"
                               {{ old('estado', $album->estado) === $val ? 'checked' : '' }}>
